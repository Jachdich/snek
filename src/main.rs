...
fn highlight(n: &str, x: isize, y: isize) -> String {
    ...
    for line in n.split("\n") {
        if y == ny {
            ...
            buf.push_str(&format!(
                "{}\x1b[7m{}\x1b[0m{}\n",
                std::str::from_utf8(&by[0..x]).unwrap(),
                by[x] as char,
                std::str::from_utf8(&by[(x + 1)..]).unwrap()
            ));
        } else {
            buf.push_str(line);
            buf.push('\n');
...
impl Interpreter {
    ...
    fn move_from_score(&mut self) {
        ...
        if self.dir == Dir::E {
            ...
            if self.grid.get(self.x + 1, self.y) == '/'
                && self.grid.get(self.x + 1, self.y + 1) == '\\'
            {
            ...
            } else if self.grid.get(self.x + 1, self.y) == '_' {
                self.x += 1;
            } else if self.grid.get(self.x + 1, self.y) == '@' {
                self.x += 1;
            } else if self.grid.get(self.x + 1, self.y) == '/' {
                self.x += 1;
                self.dir = Dir::NE;
            } else if self.grid.get(self.x + 1, self.y + 1) == '\\' {
                self.x += 1;
                self.y += 1;
                self.dir = Dir::SE;
            } else {
                panic!(
                    "Unexpected end of path at {}, {} (from score E)",
                    self.x, self.y
                );
            }
                ...
                && self.grid.get(self.x - 1, self.y + 1) == '/'
            {
                ...
                panic!(
                    "Unexpected end of path at {}, {} (from score W)",
                    self.x, self.y
                );
            }
        } else {
    ...
    fn move_from_slash(&mut self) {
        ...
        */
        if self.dir == Dir::NE {
            ...
            //  /
            // /
            if self.grid.get(self.x, self.y - 1) == '|'
                && self.grid.get(self.x + 1, self.y - 1) == '_'
            {
                if self.stack.pop() == Some(0) {
                    self.x += 1;
                    self.y -= 1;
                ...
                self.x += 1;
                self.y -= 1;
            } else if self.grid.get(self.x, self.y - 1) == '|' {
                self.y -= 1;
                self.dir = Dir::N;
            } else if self.grid.get(self.x + 1, self.y - 1) == '|' {
                self.x += 1;
                self.y -= 1;
                self.dir = Dir::N;
            } else if self.grid.get(self.x + 1, self.y - 1) == '_' {
                self.x += 1;
                self.y -= 1;
                self.dir = Dir::E;
            } else {
                panic!(
                    "Unexpected end of path at {}, {} (from slash NE)",
                    self.x, self.y
                );
            }
            ...
            if self.grid.get(self.x, self.y + 1) == '|' && self.grid.get(self.x - 1, self.y) == '_'
            {
                ...
                panic!(
                    "Unexpected end of path at {}, {} (from slash SW)",
                    self.x, self.y
                );
            }
        } else {
        ...
        */
        if self.dir == Dir::SE {
            ...
            if self.grid.get(self.x + 1, self.y) == '_' && self.grid.get(self.x, self.y + 1) == '|'
            {
                ...
                panic!(
                    "Unexpected end of path at {}, {} (from backslash SE)",
                    self.x, self.y
                );
            }
            ...
            //   \
            //    \
            if self.grid.get(self.x - 1, self.y - 1) == '_'
                && self.grid.get(self.x, self.y - 1) == '|'
            {
                if self.stack.pop() == Some(0) {
                    self.y -= 1;
                    self.dir = Dir::N;
                ...
                panic!(
                    "Unexpected end of path at {}, {} (from backslash NW)",
                    self.x, self.y
                );
            }
        } else {
    ...
    fn move_from_pipe(&mut self) {
        ...
        */
        if self.dir == Dir::N {
            ...
            //  |
            //  |
            if self.grid.get(self.x - 1, self.y - 1) == '\\'
                && self.grid.get(self.x + 1, self.y - 1) == '/'
            {
                if self.stack.pop() == Some(0) {
                    self.y -= 1;
                ...
                panic!(
                    "Unexpected end of path at {}, {} (from pipe N)",
                    self.x, self.y
                );
            }
                ...
                && self.grid.get(self.x + 1, self.y + 1) == '\\'
            {
                ...
                panic!(
                    "Unexpected end of path at {}, {} (from pipe S)",
                    self.x, self.y
                );
            }
        } else {
        ...
        match ch {
            ...
            '\\' => {
                if count == 5 {
                    ...
                    print!("{}", char::from_u32(self.pop() as u32).unwrap());
                } else if count == 7 {
                    self.stack.pop();
                } else if count == 4 {
    ...
    fn run(&mut self) {
        loop {
            ...
            println!(
                "{}, {}, {}, {:?}, {:?}",
                highlight(&self.program, self.x, self.y),
                self.x,
                self.y,
                self.dir,
                self.stack
...
// TODO in debug mode, have fake i/o shell to see printing
// show current command in debug mode
// ide: move to the end of the last typed char in snekworld: \
